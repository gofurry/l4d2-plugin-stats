...
管理员可以分别配置明细、已关闭 Session 和比赛结果的保留天数（30–3650 天）。点击清理前，系统会先增量聚合，生成包含源水位与候选数量的预览 ID，校验聚合覆盖当前 Stats DB，再在执行前复核预览没有变化。删除使用独立写连接，每 500 行一个短事务分批提交，结果写入 `retention_runs` 审计表。
