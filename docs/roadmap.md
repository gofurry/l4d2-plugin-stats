...
项目已经完成数据库地基、真人身份/Session、Run/Round/Segment 生命周期，以及 v0.4.0～v0.5.3 PvE 统计的本地验收。v0.6.0 对抗核心统计、v0.6.2 感染者职业明细和 v0.6.3 控制与能力效果已经通过本地验证；v0.6.4 幸存者战斗明细和 v0.6.5 引擎比分/比赛结果完成实现和离线验证，等待本地数据验收。完整双方真人对抗、MySQL 和 PostgreSQL 实机兼容验证统一保留到 v0.7.0。
...
### v0.6.5 - Versus Scores and Match Results

**Status:** Implementation complete; local validation pending

**Scope:** User-facing / Data model / Correctness / Testing

**Goal:** 保存引擎权威的对抗半场分、战役累计分和最终结果，不自行重算计分公式。
...
- [x] 从 L4D2 GameRules 读取两个逻辑队伍的地图分和战役累计分
- [x] 按 Round 保存本半场计分槽位、队伍翻转、原始 winner 和完成/重开状态
- [x] 按 Run 保存累计分，并仅在正常完成时派生 0/1/平局胜者
- [x] 将结果快照并入统一异步事务、有界生命周期队列和异常启动恢复
- [x] 完成三数据库等价 DDL、SQLite 回归、检查器和口径文档
- [ ] 按 [v0.6.5 本地测试清单](v0.6.5-test-checklist.md) 核对一张图两个半场的实际分数
- [ ] 在最终章节两个半场后核对 `completed` Run 的最终累计分和胜者

#### Acceptance Criteria

- 两个半场各产生一条可解释的 Round 结果，计分槽位不同且分数与游戏结算界面一致
- 正常完成的 Run 能区分逻辑队伍 0/1、平局和分数不可用；异常结束不伪造胜者
- 半场重开会把上一次相同半场结果修正为 `abandoned`，不会伪装成两个正常半场
- 不产生逐 tick/逐事件 SQL，不引入 Left4DHooks 硬依赖，也不按坐标自行推算推进分

#### Notes

一张普通章节的上下半场足以完成核心验收；推荐连续完成战役最后两章的上下半场，以同时
验证跨图累计分和最终胜者。不要求从第一章重打完整长战役。跨 Run 稳定队伍、逐时刻
推进曲线、第三方计分插件分项和 MVP 不属于本阶段。

---

### v0.6.6 - Versus Contract Freeze
...
- [ ] 明确稳定队伍、连续推进曲线、第三方计分分项和 MVP 继续延期
...
- 原始事件流水和逐事件回放
- 助攻、控制链及高级技术动作
- 对抗跨 Run 稳定队伍归属、逐时刻推进曲线、第三方计分插件分项和 MVP
