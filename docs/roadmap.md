...
当前版本为 `v1.3.4`。SourceMod 采集器使用 Stats schema 6 / `stats_version = 1`，支持 SQLite、MySQL 和 PostgreSQL；Dashboard 使用 schema 20，Aggregate Contract 与 Achievement Contract 均为 v1。Achievement Catalog 现包含 105 个底层项目和 38 个 artwork key。详细版本变化统一记录在 [`CHANGELOG.md`](../CHANGELOG.md)。
