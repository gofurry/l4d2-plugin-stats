...
- Stats schema 7's five nullable survivor telemetry fields remain outside Aggregate Contract v1. Player detail reads all five from permanent core rows; only `teammate_protections`, `hunter_skeets`, and `charger_levels` may expose raw full-server rankings. `NULL` is uncollected history and must never be coerced into a claim of full lifetime coverage.
