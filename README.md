...
项目采用 monorepo。当前实现为 v0.6.2 统计采集器：在真人身份、Session、Run、Round 和 Segment 生命周期之上，已接入 `coop`、`realism` 的完整 PvE 统计，以及独立的 `versus` 幸存者/感染者统计和感染者职业明细。插件稳定后再开发 Go 后端及其内嵌前端。
...
v0.6.0 的核心归属、单人 Bot 上下半场和跨图 Run/Session 连续性已经通过本地验收；v0.6.2 职业明细完成实现与离线验证，等待本地对抗数据验收。完整的双方真人、换队、Tank 交接、重连和半场重开统一延期到 v0.7.0。当前不推算稳定队伍、推进分、胜负或 MVP；设备类别和总计将在未来 Go 查询侧从精确设备行聚合。
