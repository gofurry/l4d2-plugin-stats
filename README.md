...
项目采用 monorepo。当前实现为 v0.5.2 PvE 统计采集器：在真人身份、Session、Run、Round 和 Segment 生命周期之上，已接入 `coop` 与 `realism` 的战斗、救援、治疗、章节成绩、设备明细、控制/解救、技巧、目标互动和状态时长统计。插件稳定后再开发 Go 后端及其内嵌前端。
...
v0.5.2 **尚不采集 Versus 玩法统计**；设备类别和总计将在未来 Go 查询侧从精确设备行聚合。
