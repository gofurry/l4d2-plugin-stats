...
v0.6.0 的实现、离线数据库验证和单人 Bot 上下半场统计已经完成；第二半场到下一地图的 Run/Session 连续性修复等待本地回归。完整的双方真人、换队、Tank 交接、重连和半场重开统一延期到 v0.7.0。第一阶段不推算稳定队伍、推进分、胜负或 MVP；设备类别和总计将在未来 Go 查询侧从精确设备行聚合。
