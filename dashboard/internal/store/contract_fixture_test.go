...
func insertContractFixture(t *testing.T, db *sql.DB) {
	...
	w := contractWatermark
	statements := []string{
		fmt.Sprintf(`INSERT INTO lps_schema_migrations (version,name,applied_at) VALUES (1,'initial',%d),(2,'car_alarms_triggered',%d),(3,'versus_objective_interactions',%d),(4,'analysis_foundation',%d),(5,'relationships_and_assists',%d),(6,'fall_deaths',%d),(7,'high_value_telemetry_chat',%d)`, b, b+1, b+2, b+3, b+4, b+5, b+6),
		...
		fmt.Sprintf(`INSERT INTO lps_sessions (session_id,boot_id,server_key,steam_id,player_name,ip_address,started_at,ended_at,last_saved_at,connected_seconds,active_play_seconds,status,disconnect_reason) VALUES ('session-alice','boot-one','one','1','Alice','127.0.0.1',%d,%d,%d,300,240,'closed','quit'),('session-bob','boot-one','one','2','Bob','127.0.0.2',%d,%d,%d,100,60,'closed','timeout')`, b, b+300, w, b+5, b+105, b+300),
		...
		fmt.Sprintf(`INSERT INTO lps_player_segments (segment_id,session_id,run_id,round_id,server_key,steam_id,side,started_at,ended_at,last_saved_at,active_play_seconds,status) VALUES ('segment-pve','session-alice','run-pve','round-pve','one','1','survivor',%d,%d,%d,90,'closed'),('segment-vs','session-alice','run-versus','round-versus','one','1','survivor',%d,%d,%d,70,'closed'),('segment-vi','session-alice','run-versus','round-versus','one','1','infected',%d,%d,%d,60,'closed'),('segment-pve-history','session-bob','run-pve','round-pve','one','2','survivor',%d,%d,%d,0,'closed')`, b+30, b+180, b+430, b+40, b+200, b+440, b+50, b+210, b+450, b+35, b+175, b+432),
		fmt.Sprintf(`INSERT INTO lps_pve_segment_stats (segment_id,stats_version,last_saved_at,common_kills,special_kills,tank_kills,witch_kills,damage_to_special,damage_to_tank,damage_to_witch,damage_taken_infected,friendly_fire_to_humans,friendly_fire_to_bots,friendly_fire_taken,incapacitations,deaths,incap_revives,ledge_rescues,defib_revives,rescues_received,medkits_used_self,medkits_used_on_others,medkit_healing_self,medkit_healing_others,pills_used,adrenaline_used,temporary_health_received,chapter_participations,chapter_completions_alive,campaign_completions,smoker_kills,damage_to_smoker,smoker_controls_received,smoker_controlled_seconds,smoker_saves,melee_tongue_self_cuts,tank_rocks_destroyed,witch_oneshots,witch_solo_kills,tank_encounters,tank_kill_participations,witch_encounters,witch_kill_participations,incendiary_packs_deployed,explosive_packs_deployed,objective_interactions,ammo_pile_uses,incapacitated_seconds,ledge_hanging_seconds,black_white_teammates_restored,car_alarms_triggered,special_assists,smoker_assists,boomer_assists,hunter_assists,spitter_assists,jockey_assists,charger_assists,fall_deaths,teammate_protections,ledge_grabs,tank_rock_hits_received,hunter_skeets,charger_levels) VALUES ('segment-pve',1,%d,100,12,2,1,1200,400,200,300,10,5,7,3,1,2,3,4,1,2,1,80,40,3,2,50,1,1,1,4,500,2,30,3,1,2,1,1,2,2,1,1,1,1,2,4,20,10,1,3,6,1,1,1,1,1,1,1,4,2,1,3,1)`, b+460),
		fmt.Sprintf(`INSERT INTO lps_pve_segment_stats (segment_id,stats_version,last_saved_at) VALUES ('segment-pve-history',1,%d)`, b+100),
		...
		fmt.Sprintf(`INSERT INTO lps_versus_survivor_stats (segment_id,stats_version,last_saved_at,common_kills,human_special_kills,bot_special_kills,human_tank_kills,bot_tank_kills,damage_to_human_special,damage_to_bot_special,damage_to_human_tank,damage_to_bot_tank,damage_taken_infected,friendly_fire_to_humans,friendly_fire_to_bots,friendly_fire_taken,incapacitations,deaths,incap_revives,ledge_rescues,defib_revives,rescues_received,medkits_used_self,medkits_used_on_others,medkit_healing_self,medkit_healing_others,pills_used,adrenaline_used,temporary_health_received,witch_kills,damage_to_witch,molotovs_thrown,pipe_bombs_thrown,vomit_jars_thrown,incendiary_packs_deployed,explosive_packs_deployed,melee_tongue_self_cuts,tank_rocks_destroyed,witch_oneshots,witch_solo_kills,objective_interactions,car_alarms_triggered,human_special_assists,bot_special_assists,human_tank_assists,bot_tank_assists,witch_encounters,witch_kill_participations,black_white_teammates_restored,fall_deaths,teammate_protections,ledge_grabs,tank_rock_hits_received,hunter_skeets,charger_levels) VALUES ('segment-vs',1,%d,40,7,3,2,1,100,50,200,75,80,4,2,3,2,1,1,2,1,2,1,2,30,50,2,1,20,1,90,1,2,3,1,2,1,1,1,1,5,2,4,2,1,1,2,1,1,0,2,1,1,1,1)`, b+470),
		fmt.Sprintf(`INSERT INTO lps_versus_survivor_infected_class_stats (segment_id,infected_class,stats_version,last_saved_at,human_controller_kills,bot_controller_kills,damage_to_human_controllers,damage_to_bot_controllers,human_controller_assists,bot_controller_assists) VALUES ('segment-vs',3,1,%d,5,2,120,60,4,2),('segment-vs',8,1,%d,2,1,200,75,1,1)`, b+471, b+471),
		fmt.Sprintf(`INSERT INTO lps_player_round_relationship_stats (round_id,actor_steam_id,target_steam_id,relationship_version,incap_revives,ledge_rescues,defib_revives,hunter_rescues,control_rescue_duration_ms,medkits_used,medkit_healing,black_white_restores,friendly_fire_damage,last_saved_at,revision) VALUES ('round-pve','1','2',1,2,1,1,3,2400,2,75,1,5,%d,1),('round-pve','2','1',1,1,0,0,1,900,1,30,0,2,%d,1)`, b+472, b+472),
