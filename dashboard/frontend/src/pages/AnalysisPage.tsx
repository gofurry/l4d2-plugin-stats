...
export function AnalysisPage() {
  ...
  const contextColumns = [
    { title: zh ? '规则配置' : 'Rules', render: (_: unknown, row: AnalysisContextRow) => <strong>{row.ruleset_name || row.difficulty || (zh ? '默认规则配置' : 'Default rules')}</strong> },
    ...
    { title: zh ? '游戏参数' : 'Game settings', render: (_: unknown, row: AnalysisContextRow) => zh ? `${row.difficulty || '未知难度'} · 幸存者 ${row.survivor_limit} · 特感玩家上限 ${row.max_player_zombies} · 普通感染者上限 ${row.common_limit} · Tank 生命 ${row.tank_health} · Witch 生命 ${row.witch_health}` : `${row.difficulty || 'Unknown difficulty'} · ${row.survivor_limit} survivors · ${row.max_player_zombies} player SI · ${row.common_limit} common limit · Tank ${row.tank_health} HP · Witch ${row.witch_health} HP` },
  ]
  ...
  return <Layout className={styles.layout}><FloatingNav/><Layout.Content className={styles.content}>
    ...
    {tab === 'maps' && <>{maps.isLoading ? <State loading/> : maps.data ? <>
      ...
      <p className={styles.availability}>{zh?'战局事件数据覆盖日期':'Battle-event data coverage'}: {maps.data.earliest_incident_at ? new Date(maps.data.earliest_incident_at*1000).toLocaleDateString() : '—'} - {maps.data.latest_incident_at ? new Date(maps.data.latest_incident_at*1000).toLocaleDateString() : '—'} · v{maps.data.incident_version}</p>
