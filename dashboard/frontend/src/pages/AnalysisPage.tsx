...
export function AnalysisPage() {
  ...
  return <Layout className={styles.layout}><FloatingNav/><Layout.Content className={styles.content}>
    <section className={styles.controlCard}><div className={styles.filterBar}><Segmented value={range} onChange={value => { setRange(String(value)); setMapPage(1); setContextPage(1) }} options={[['30d', zh ? '近 30 天' : '30d'], ['90d', zh ? '近 90 天' : '90d'], ['180d', zh ? '近 180 天' : '180d'], ['all', zh ? '全部' : 'All']].map(([value,label])=>({value,label}))}/><div className={styles.filters}><Select value={mode} onChange={value => { setMode(value); setServer(undefined); setCampaign(undefined); setMapPage(1); setContextPage(1) }} options={[{value:'pve',label:'PvE'},{value:'versus',label:zh?'对抗':'Versus'}]}/><Select allowClear showSearch value={server} onChange={value => { setServer(value); setMapPage(1); setContextPage(1) }} placeholder={zh?'全部服务器':'All servers'} options={options.data?.servers.map(value=>({value,label:value}))}/><Select allowClear showSearch value={campaign} onChange={value => { setCampaign(value); setMapPage(1); setContextPage(1) }} placeholder={zh?'全部战役':'All campaigns'} options={options.data?.campaigns.map(value=>({value,label:value}))}/></div></div><Tabs className={styles.tabs} activeKey={tab} onChange={setTab} items={[{key:'maps',label:zh?'地图与战役':'Maps & campaigns'},{key:'contexts',label:zh?'规则环境':'Rule contexts'}]}/></section>
