...
import { api, type AchievementBadge as Badge, type AchievementCard, type PlayerAchievements as AchievementData } from '../../api'
...
export function PlayerAchievements({ steamID, data, loading, self, zh }: { steamID: string; data?: AchievementData; loading: boolean; self: boolean; zh: boolean }) {
  ...
  const toggleBadge = (card: AchievementCard) => {
    if (!self || !card.unlocked || !card.artwork_key) return
  ...
  return <div className={styles.achievementStack}>
    <section className={styles.achievementOverview}>
      <div className={styles.achievementProgress}><div><span>{zh ? '成就进度' : 'Achievement progress'}</span><strong>{data.overview.unlocked} / {data.overview.total}</strong></div><Progress percent={Number(data.overview.completion_percent.toFixed(1))} strokeColor="#d4763b" trailColor="rgba(127,101,83,.16)" /></div>
      <div className={styles.achievementSummary}><span>{zh ? '完成度' : 'Completion'}<strong>{data.overview.completion_percent.toFixed(1)}%</strong></span><span>{zh ? '发现彩蛋' : 'Easter eggs'}<strong>{data.overview.easter_eggs}</strong></span><span>{zh ? '最近解锁' : 'Latest unlock'}<strong>{latestTitle(data)}</strong></span></div>
      <div className={styles.badgeShowcase}><div><strong>{zh ? '徽章展示' : 'Badge showcase'}</strong><small>{self ? (zh ? '点击已解锁成就可调整，最多 3 个' : 'Select up to three unlocked achievements') : (zh ? '玩家展示的徽章' : 'Player showcase')}</small></div><div>{badges.map(item => <AchievementBadge key={item.achievement_key} artworkKey={isAchievementArtworkKey(item.artwork_key) ? item.artwork_key : undefined} tier={item.tier} size={46} label={item.title} />)}{badges.length === 0 && <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={zh ? '暂无徽章' : 'No badges'} />}</div></div>
    </section>

    <section className={styles.achievementFilters}><Segmented block value={category} onChange={value => setCategory(String(value))} options={[
    ...
    ].map(([value, label]) => ({ value, label }))} /></section>
    ...
    <div className={styles.achievementGrid}>{groups.map(group => {
      ...
      return <article key={card.group_key || card.achievement_key} className={`${styles.achievementCard} ${card.unlocked ? styles.achievementUnlocked : ''}`}>
        ...
        {self && equip && <Button className={styles.badgeAction} size="small" type={selected ? 'primary' : 'default'} icon={<PushpinOutlined />} loading={save.isPending} onClick={() => toggleBadge(equip)}>{selected ? (zh ? '已展示' : 'Shown') : (zh ? '展示徽章' : 'Show badge')}</Button>}
      </article>
    })}</div>
  </div>
}
...
function formatMetric(value = 0, metric = '') {
  if (metric.includes('seconds')) return `${(value / 3600).toFixed(value >= 36000 ? 0 : 1)} h`
  if (metric.includes('damage') || metric.includes('healing')) return `${compact.format(value)} HP`
  return value >= 10000 ? compact.format(value) : number.format(value)
}
