...
export function PlayerPVE({ data, loading, details, copy, zh }: { data?: PlayerPVEOverview | PlayerPVEDetails; loading: boolean; details?: boolean; copy: PlayerCopy; zh: boolean }) {
  ...
  return <div className={styles.sectionGrid}>
    ...
    <Section title={copy.skills}><MetricList items={[[zh ? 'Hunter 空中击杀' : 'Hunter Skeets', nullable(overview.hunter_skeets, zh)], [zh ? '近战截停 Charger' : 'Charger Levels', nullable(overview.charger_levels, zh)], [copy.rocks, overview.tank_rocks_destroyed], [zh ? '被 Tank 石块命中' : 'Tank rock hits received', nullable(overview.tank_rock_hits_received, zh)], [copy.tongue, overview.melee_tongue_self_cuts], [copy.witchOneShot, overview.witch_oneshots], [copy.witchSolo, overview.witch_solo_kills]]} /></Section>
...
function PVEDetails({ data, copy, zh }: { data: PlayerPVEDetails; copy: PlayerCopy; zh: boolean }) {
  ...
  const classColumns = [
    ...
    { title: zh ? '助攻' : 'Assists', dataIndex: 'assists', key: 'assists', render: (value: number | null) => nullable(value, zh) },
