...
def main() -> None:
    ...
    try:
        ...
        invalid_versus_infected_class_stats = database.execute(
            ...
            "bot_survivor_kills, human_survivor_controls, "
            "bot_survivor_controls, human_survivor_control_seconds, "
            "bot_survivor_control_seconds, human_survivor_ability_hits, "
            "bot_survivor_ability_hits, human_survivor_ability_damage, "
            "bot_survivor_ability_damage, revision) < 0"
        ).fetchone()[0]
        invalid_versus_infected_ability_stats = database.execute(
            "SELECT COUNT(*) FROM lps_versus_infected_class_stats WHERE "
            "(infected_class NOT IN (1, 3, 5, 6) AND "
            "(human_survivor_controls > 0 OR bot_survivor_controls > 0 OR "
            "human_survivor_control_seconds > 0 OR "
            "bot_survivor_control_seconds > 0)) OR "
            "(infected_class <> 2 AND (human_survivor_ability_hits > 0 OR "
            "bot_survivor_ability_hits > 0)) OR "
            "(infected_class <> 4 AND (human_survivor_ability_damage > 0 OR "
            "bot_survivor_ability_damage > 0)) OR "
            "(human_survivor_control_seconds > 0 AND "
            "human_survivor_controls = 0) OR "
        ...
        rows = database.execute(
            ...
            "c.human_survivor_kills, c.bot_survivor_kills, "
            "c.human_survivor_controls, c.bot_survivor_controls, "
            "c.human_survivor_control_seconds, c.bot_survivor_control_seconds, "
            "c.human_survivor_ability_hits, c.bot_survivor_ability_hits, "
            "c.human_survivor_ability_damage, c.bot_survivor_ability_damage, "
            "c.revision "
        ...
        for row in rows:
            print(
                ...
                f"kills_human={row[8]} kills_bot={row[9]} "
                f"controls_human={row[10]} controls_bot={row[11]} "
                f"control_seconds_human={row[12]} control_seconds_bot={row[13]} "
                f"ability_hits_human={row[14]} ability_hits_bot={row[15]} "
                f"ability_damage_human={row[16]} ability_damage_bot={row[17]} "
                f"revision={row[18]}"
            )
    finally:
        database.close()
